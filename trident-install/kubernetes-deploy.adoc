...
無論手動部署或使用 Helm，Trident operator 都是簡化安裝和動態管理 Trident 資源的絕佳方式。您甚至可以link:../trident-install/kubernetes-customize-deploy.html["自訂您的Trident營運者部署"]使用 `TridentOrchestrator`自訂資源 （CR） 中的屬性。
